...
void swap(int a, int b,int *route);
double hillclimbing(const City *city, int n,int *route);
...
int main(int argc, char**argv)
{
  ...
  printf("0\n");
  const double d1 = hillclimbing(city, n, route);
  plot_cities(fp, map, city, n, route);
  printf("the shortest total distance = %f\n", d1);
  for (int i = 0 ; i < n ; i++){
    printf("%d -> ", route[i]);
  }
  printf("%d\n", route[0]);
...
double hillclimbing(const City *city, int n,int route[]){//hill climbing
    for(int i = 0; i < 1000000; i++){
        ...
        for(int i=0; i<n; i++){
          ...
          nowroad += distance(city[c0],city[c1]);
        }
        swap(p1,p2,route);
        double newroad = 0;
        for(int i=0; i<n; i++){
          const int c0 = route[i];
          const int c1 = route[(i+1)%n];
          newroad += distance(city[c0],city[c1]);
        }
        if(newroad >= nowroad)
           swap(p1,p2,route);
    }
