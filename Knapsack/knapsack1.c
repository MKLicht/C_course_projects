...
Itemset *load_itemset(char *filename)
{
  ...
  fread(&(items->number),sizeof(int),1,fp);
  int n = items->number;
  items->item = (Item*)malloc(sizeof(Item) * n);
  for (int i = 0 ; i < n ; i++){
    fread(&(items->item[i].value), sizeof(double), 1, fp);
...
int main (int argc, char**argv)
{
  ...
  Itemset *items = load_itemset(argv[3]);
  assert( n <= items->number );
...
double search(int index, const Itemset *list, double capacity, int *flags, double sum_v, double sum_w)
{
  ...
  assert(index >= 0 && sum_v >= 0 && sum_w >= 0 && sum_w <= capacity);
