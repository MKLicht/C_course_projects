...
int main(int argc, char **argv)
{
  ...
  objects[3] = (Object){ .m = 100000.0, .x = 1000, .y = 1000, .vy = 0.0};
...
void my_update_positions(Object objs[4], const size_t objnum, const Condition cond)
{
...
void my_plot_objects(Object objs[4], const size_t objnum, const double t, const Condition cond)
{
  printf("+");
  ...
  for(int y = -(int)(cond.height/2) ; y < (int)(cond.height/2) ; y++) 
  {
      ...
      for (int x = -(int)(cond.width/2) ; x < (int)(cond.width/2) ; x++)
      {
          ...
          for(int i=0;i<objnum-1;i++)
          {
            if(x==(int)(objs[i].x) && y==(int)(objs[i].y))
            {
              printf("o");
              flag=1;
              break;
